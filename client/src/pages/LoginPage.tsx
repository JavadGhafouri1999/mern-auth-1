...
export default function LoginPage() {
	...
	return (
		<div className="rounded-xl text-gray-300 w-full max-w-5xl backdrop-blur-xs flex justify-between gap-4 mx-5 lg:mx-auto p-4 bg-white/10">
			{/* Right - Form */}
			<div className="min-h-full w-full md:w-1/2 flex flex-col gap-12 p-4 rounded-xl overflow-hidden">
				...
				<h1 className="w-full text-xl md:text-2xl font-bold text-center mt-4 md:mt-8">
					ورود به حساب
				</h1>
				<form className="h-full flex flex-col items-start gap-8 w-full">
					<div className="flex flex-col gap-4 w-full max-w-xs mx-auto">
						<FormControl>
							<FormLabel className="text-sm text-gray-200">آدرس ایمیل</FormLabel>
							<Input
								placeholder="user@example.com"
								variant="soft"
								className="w-full mx-auto ltr"
							/>
						</FormControl>
						<FormControl>
							<FormLabel className="text-sm text-gray-200">رمز عبور</FormLabel>
							<Input
								placeholder="*********"
								type={showPass ? "text" : "password"}
								variant="soft"
								className="w-full mx-auto ltr relative pr-6"
							/>
							{showPass ? (
								<VisibilityOffIcon
									sx={{ fontSize: 15 }}
					...
					<Button variant="outlined" className="w-full max-w-xs mx-auto">
						ورود به حساب
					</Button>
				</form>
				<div className="flex items-center gap-2 text-sm self-baseline mt-auto">
			...
			{/* Left- Image */}
			<div className="hidden md:block w-1/2 overflow-hidden rounded-xl">
				<img src="./hands.jpg" alt="skull" className="rounded-xl object-cover" />
			</div>
		</div>
	);
}
